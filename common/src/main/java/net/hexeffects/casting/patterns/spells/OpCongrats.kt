package net.hexeffects.casting.patterns.spells
...
import net.hexeffects.networking.SetLookPitchS2CMsg
...
class OpCongrats : SpellAction {
    ...
    override fun execute(args: List<Iota>, ctx: CastingContext): Triple<RenderedSpell, Int, List<ParticleSpray>> {
        ...
        if (target !is ServerPlayer) throw MishapBadEntity(
            target,
            Component.translatable("text.hexeffects.congrats.player")
        )
    ...
    private data class Spell(val player: ServerPlayer) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            player.sendSystemMessage(Component.translatable("text.hexeffects.congrats", player.displayName));
            HexEffectsNetworking.sendToPlayer(player, SetLookPitchS2CMsg(-90f))
        }
    }
}
